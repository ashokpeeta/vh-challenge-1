...
describe("SpellingFixer", () => {
  const makeGetMock = (params: any) => {
    mockAxios.get.mockImplementation((url: string, options: any): Promise<any> => {
      if (url.includes("corrections") && options.params &&
  ...
  const makePostMock = (mockResponse: any, uncorrectedText: string) => {
    mockAxios.post.mockImplementationOnce((url: string, params: any): Promise<any> => {
      if (url.includes("checkspelling") && params && 
  ...
  for (const mock of mockResponses) {
    it(`should correct "${mock.corpus}" -> "${mock.expected}"`, async () => {
      ...
      expect(mockAxios.get).toHaveBeenCalledTimes(Object.values(mock.GET).length);
